...
static void Set_Clock_Delay(unsigned int clock) {
  unsigned int delay;
...
static unsigned int DAP_Delay(const uint8_t *request, uint8_t *response) {
  unsigned int delay;

  delay  = (unsigned int)(*(request+0)) |
           (unsigned int)(*(request+1) << 8);
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static unsigned int DAP_HostStatus(const uint8_t *request, uint8_t *response) {
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static unsigned int DAP_Connect(const uint8_t *request, uint8_t *response) {
  unsigned int port;
...
static unsigned int DAP_Disconnect(uint8_t *response) {
...
//   response: pointer to response data
//   return:   number of bytes in response
static unsigned int DAP_ResetTarget(uint8_t *response) {
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static unsigned int DAP_SWJ_Pins(const uint8_t *request, uint8_t *response) {
  ...
  value  = (unsigned int) *(request+0);
  select = (unsigned int) *(request+1);
  wait   = (unsigned int)(*(request+2) <<  0) |
           (unsigned int)(*(request+3) <<  8) |
           (unsigned int)(*(request+4) << 16) |
           (unsigned int)(*(request+5) << 24);
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static unsigned int DAP_SWJ_Clock(const uint8_t *request, uint8_t *response) {
#if ((DAP_SWD != 0) || (DAP_JTAG != 0))
  unsigned int clock;
  unsigned int delay;

  clock = (unsigned int)(*(request+0) <<  0) |
          (unsigned int)(*(request+1) <<  8) |
          (unsigned int)(*(request+2) << 16) |
          (unsigned int)(*(request+3) << 24);
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static unsigned int DAP_SWJ_Sequence(const uint8_t *request, uint8_t *response) {
  unsigned int count;
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static unsigned int DAP_SWD_Configure(const uint8_t *request, uint8_t *response) {
#if (DAP_SWD != 0)
  uint8_t value;
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static unsigned int DAP_SWD_Sequence(const uint8_t *request, uint8_t *response) {
  unsigned int sequence_info;
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static unsigned int DAP_JTAG_Sequence(const uint8_t *request, uint8_t *response) {
  unsigned int sequence_info;
  unsigned int sequence_count;
  unsigned int request_count;
  unsigned int response_count;
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static unsigned int DAP_JTAG_Configure(const uint8_t *request, uint8_t *response) {
  unsigned int count;
#if (DAP_JTAG != 0)
  unsigned int length;
  unsigned int bits;
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static unsigned int DAP_JTAG_IDCode(const uint8_t *request, uint8_t *response) {
#if (DAP_JTAG != 0)
  unsigned int data;
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static unsigned int DAP_TransferConfigure(const uint8_t *request, uint8_t *response) {
...
//             number of bytes in request (upper 16 bits)
#if (DAP_SWD != 0)
static unsigned int DAP_SWD_Transfer(const uint8_t *request, uint8_t *response) {
  const
  ...
  while (request_count != 0) {
    ...
    if ((request_value & DAP_TRANSFER_RnW) != 0U) {
      ...
      if ((request_value & DAP_TRANSFER_MATCH_VALUE) != 0U) {
        ...
        match_value = (unsigned int)(*(request+0) <<  0) |
                      (unsigned int)(*(request+1) <<  8) |
                      (unsigned int)(*(request+2) << 16) |
                      (unsigned int)(*(request+3) << 24);
        request += 4;
  ...
  *(response_head+1) = (uint8_t)response_value;
  ...
  return (((unsigned int)(request - request_head) << 16) | (unsigned int)(response - response_head));
}
#endif
...
//             number of bytes in request (upper 16 bits)
#if (DAP_JTAG != 0)
static unsigned int DAP_JTAG_Transfer(const uint8_t *request, uint8_t *response) {
  const
  uint8_t  *request_head;
  unsigned int  request_count;
  unsigned int  request_value;
  unsigned int  request_ir;
  uint8_t  *response_head;
  unsigned int  response_count;
  unsigned int  response_value;
  unsigned int  post_read;
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static unsigned int DAP_Dummy_Transfer(const uint8_t *request, uint8_t *response) {
  const
  uint8_t  *request_head;
  unsigned int  request_count;
  unsigned int  request_value;
  ...
  *(response+1) = 0U;   // Response value
  ...
  return (((unsigned int)(request - request_head) << 16) | 2U);
}
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static unsigned int DAP_Transfer(const uint8_t *request, uint8_t *response) {
  unsigned int num;
...
//   return:   number of bytes in response
#if (DAP_SWD != 0)
static unsigned int DAP_SWD_TransferBlock(const uint8_t *request, uint8_t *response) {
  unsigned int  request_count;
  unsigned int  request_value;
  unsigned int  response_count;
  unsigned int  response_value;
  ...
  request_count = (unsigned int)(*(request+0) << 0) |
                  (unsigned int)(*(request+1) << 8);
  request += 2;
  if (request_count == 0U) {
    goto end;
  ...
  *(response_head+2) = (uint8_t) response_value;
  ...
  return ((unsigned int)(response - response_head));
}
#endif
...
//   return:   number of bytes in response
#if (DAP_JTAG != 0)
static unsigned int DAP_JTAG_TransferBlock(const uint8_t *request, uint8_t *response) {
  unsigned int  request_count;
  unsigned int  request_value;
  unsigned int  response_count;
  unsigned int  response_value;
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static unsigned int DAP_TransferBlock(const uint8_t *request, uint8_t *response) {
  unsigned int num;
  ...
  if ((*(request+3) & DAP_TRANSFER_RnW) != 0U) {
    ...
    // Write register block
    num |= (4U + (((unsigned int)(*(request+1)) | (unsigned int)(*(request+2) << 8)) * 4)) << 16;
  }
...
//   return:   number of bytes in response
#if (DAP_SWD != 0)
static unsigned int DAP_SWD_WriteAbort(const uint8_t *request, uint8_t *response) {
  unsigned int data;

  // Load data (Ignore DAP index)
  data = (unsigned int)(*(request+1) <<  0) |
         (unsigned int)(*(request+2) <<  8) |
         (unsigned int)(*(request+3) << 16) |
         (unsigned int)(*(request+4) << 24);
...
//   return:   number of bytes in response
#if (DAP_JTAG != 0)
static unsigned int DAP_JTAG_WriteAbort(const uint8_t *request, uint8_t *response) {
  unsigned int data;
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static unsigned int DAP_WriteAbort(const uint8_t *request, uint8_t *response) {
  unsigned int num;
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
__attribute__((weak)) unsigned int DAP_ProcessVendorCommand(const uint8_t *request, uint8_t *response) {
  (void)request;
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
unsigned int DAP_ProcessCommand(const uint8_t *request, uint8_t *response) {
  unsigned int num;
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
unsigned int DAP_ExecuteCommand(const uint8_t *request, uint8_t *response) {
  unsigned int cnt, num, n;
